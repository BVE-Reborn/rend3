...
pub fn create_renderer(iad: InstanceAdapterDevice) -> Result<Arc<Renderer>, RendererInitializationError> {
    let features = iad.device.features();

    let camera_manager = RwLock::new(CameraManager::new(Camera::default(), None));
