...
pub struct MeshBuilder {
    ...
    vertex_joint_indices: Option<Vec<UVec4>>,
...
impl MeshBuilder {
    ...
    ///
    /// Will panic if the length is different from the position buffer length.
    pub fn with_vertex_joint_indices(mut self, joint_indices: Vec<UVec4>) -> Self {
    ...
    pub fn build(self) -> Result<Mesh, MeshValidationError> {
        ...
        let mut mesh = Mesh {
            ...
            vertex_joint_indices: self.vertex_joint_indices.unwrap_or_else(|| vec![UVec4::ZERO; length]),
...
pub struct Mesh {
    ...
    pub vertex_joint_indices: Vec<UVec4>,
