...
fn main_1() {
    ...
    if ((_e116 & 8192u) != 0u) {
        ...
        if (((_e118 >> bitcast<u32>(1)) & 1u) != 0u) {
            ...
            if ((_e116 & 8u) != 0u) {
                ...
                phi_2525_ = vec3<f32>(_e183.x, _e183.y, sqrt(fma(-(_e183.y), _e183.y, fma(-(_e183.x), _e183.x, 1.0))));
            } else {
    ...
    if ((_e432 & 8192u) != 0u) {
        ...
        loop {
            ...
            if (_e442 < _e445) {
                ...
                if _e485 {
                    ...
                    let _e505 = textureSampleCompareLevel(shadow, shadow_sampler, vec2<f32>(_e464.x, _e464.y), i32(_e464.z), _e455.z, vec2<i32>(0, -1));
                    let _e512 = textureSampleCompareLevel(shadow, shadow_sampler, vec2<f32>(_e464.x, _e464.y), i32(_e464.z), _e455.z, vec2<i32>(1, 0));
                    let _e519 = textureSampleCompareLevel(shadow, shadow_sampler, vec2<f32>(_e464.x, _e464.y), i32(_e464.z), _e455.z, vec2<i32>(-1, 0));
                    phi_2843_ = fma(_e519, 0.20000000298023224, fma(_e512, 0.20000000298023224, fma(_e505, 0.20000000298023224, fma(_e491, 0.20000000298023224, (_e497 * 0.20000000298023224)))));
                }
                let _e522 = phi_2843_;
                ...
                let _e559 = fma(fma(_e551, _e556, -(_e551)), _e551, 1.0);
                local_3 = (_e440 + ((((_e419 * 0.31830987334251404) + (((_e425 + ((vec3<f32>(clamp(dot(_e425, vec3<f32>(16.5, 16.5, 16.5)), 0.0, 1.0)) - _e425) * pow((1.0 - clamp(dot(_e542, _e544), 0.0, 1.0)), 5.0))) * ((_e556 / ((3.1415927410125732 * _e559) * _e559)) * (0.5 / fma(_e549, sqrt(fma(fma((-9.999999747378752e-6 - _e546), _e556, _e547), _e547, _e556)), (_e547 * sqrt(fma(fma(-(_e549), _e556, _e549), _e549, _e556))))))) * 1.0)) * _e527) * (_e549 * (_e522 * _e429))));
                continue;
            } else {
                break;
            }
        ...
        let _e596 = local_1;
        let _e599 = local_2;
        let _e604 = unnamed.uniforms.ambient;
        o_color = max(vec4<f32>(_e593.x, _e596.y, _e599.z, _e164.w), (_e604 * _e164));
    }
    return;
}

@fragment 
fn main(@location(3) i_coords0_: vec2<f32>, @location(5) i_color: vec4<f32>, @location(1) i_normal: vec3<f32>, @location(2) i_tangent: vec3<f32>, @location(0) i_view_position: vec4<f32>,   @location(4) member_6: vec2<f32>,    @location(6) @interpolate(flat) member_7: u32, @builtin(position) @invariant l_Position: vec4<f32>,   
) -> @location(0) vec4<f32> {
